...
```{r}
# LINEAR REGRESSION : only snap dummy
fit_lm <- df %>% 
...
```{r}
# LINEAR REGRESSION : event dummy 1st try
fit_all_lm <- df %>%
  model(TSLM(
      tot_sales ~ trend() +
        season() + snap +
...
future_test <-
  cbind(new_data(total_ts_sales, n = 28), df[847:874, 5:36]) %>% mutate(
    snap =
      ifelse(
        day(df[847:874, ]$date) == 1 |
          day(df[847:874, ]$date) == 3 |
...
```{r}
#Just to check that no evidence against white noise is detected
fit_ARIMA1 %>% gg_tsresiduals()
fit_ARIMA1_1 %>% gg_tsresiduals()
```
...
```{r}
#check the accuracy of the ARIMA model
beer_fit <- df %>% filter_index(. ~ "2015-12-31") %>% 
          model(ARIMA(tot_sales ~ trend() + season() + snap +
        `name_1_Chanukah End` +
        name_1_Christmas +
        `name_1_Cinco De Mayo`+
...
beer_train <- df %>% filter_index(. ~ "2015-12-31")

beer_fit <- beer_train %>% 
  model(ARIMA(tot_sales ~ trend() + season() + snap))

future_beer <-
  cbind(new_data(beer_train, n = 28), df[847:874, 5:36]) %>% mutate(
    snap =
      ifelse(
        day(df[847:874, ]$date) == 1 |
          day(df[847:874, ]$date) == 3 |
...
beer_fc <- beer_fit %>%
  forecast(new_data = future_beer)

beer_fc %>%
  autoplot(
    df %>% filter_index("2016")
  )
```

```{r}
View(bind_rows(glance(fit_ARIMA1_1), glance(fit_lm), glance(fit_all_lm), glance(fit_generalized))
)
```
...
sales_aggr <- df2 %>%
  filter_index("2016-01-01"~"2016-01-29") %>% #remove the future cal date
  aggregate_key((dept_id / item_id), sales = sum(sales))

sales_aggr %>%
  filter(!is_aggregated(dept_id), !is_aggregated(item_id)) %>%
  filter(sales > 50) %>% 
  autoplot(sales) +
  ylab("Sales") +
...
sales_aggr %>%
  ...
  autoplot(sales) +
  facet_wrap(vars(dept_id), scales = "free_y", ncol = 2) +
  theme(legend.position = "none")
```

```{r message=TRUE, warning=TRUE}
#WARNING : sooooo much timeeee
fit_aggr <- sales_aggr %>%
  model(base = ARIMA(sales)) %>% reconcile(
    bu = bottom_up(base)
  )
...
#the model do a poor job for dep 1
