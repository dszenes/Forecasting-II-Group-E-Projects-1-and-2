...
fit_aggr <- sales_aggr %>%
  filter_index("2015-04-25"~"2015-05-23") %>% 
...
fit_aggr2 <- sales_aggr2 %>%
  filter_index("2015-04-25"~"2015-05-23") %>% 
  model(base = ARIMA(sales)) %>% reconcile(td = top_down(base))

fc2 <- fit_aggr2 %>% forecast(h = 28)
...
fc2 %>%
  filter(is_aggregated(item_id), is_aggregated(dept_id)) %>%
  accuracy(data = sales_aggr2)
