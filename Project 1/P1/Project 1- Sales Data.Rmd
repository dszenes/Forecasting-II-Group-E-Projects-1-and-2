...
cal <- cal %>% mutate(snap_TX = ifelse(cal$snap_TX == 1, TRUE,FALSE))
...
#as we've to predict for h=28 days, we've to forecast until 2016-04-23
#in order to forecast h=28, we've to take into account 4 events (+snap benefits):
#Pesach End
#OrthodoxEaster
#Cinco De Mayo
#Mother's day
...
report(fit)
fit %>%
  gg_tsresiduals() #to check if residuals are white noise
fit %>% 
  orecast(h = 28) %>%
  autoplot(monthly_sales_data) #Forecast
...
future_souvenirs <- new_data(total_ts_sales, n = 28) %>%
  mutate(snap = 
    ifelse(
      day(df2[847:874,]$date) == 1 |
        day(df2[847:874,]$date) == 3 |
        day(df2[847:874,]$date) == 5 |
        day(df2[847:874,]$date) == 6 |
        day(df2[847:874,]$date) == 7 |
        day(df2[847:874,]$date) == 9 |
        day(df2[847:874,]$date) == 11 |
        day(df2[847:874,]$date) == 12 |
        day(df2[847:874,]$date) == 13 |
        day(df2[847:874,]$date) == 15,
      "TRUE",
      "FALSE"
    )
  )
...
fit_lm %>%
  ...
  autoplot(total_ts_sales)

total_ts_sales %>%
     autoplot(tot_sales, col = "gray") +
     geom_line(data = augment(fit_lm), aes(y = .fitted), col = "blue")
#too compact, should make it wider so that can interpret it better, or maybe plot is not correct ?
```
