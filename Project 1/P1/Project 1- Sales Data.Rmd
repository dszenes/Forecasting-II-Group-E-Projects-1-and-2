...
```{r}
#sales data: from daily to monthly -> vizualization process
...
#we choose not to use this data set (price information)

cal_price <- full_join(as_tibble(cal),
                       as_tibble(prices),
                       by = "wm_yr_wk") %>%
  select(-wm_yr_wk) #merge covariates dataset
```
...
sales_fit <- total_ts_sales %>% filter_index( "2014" ~"2015-04-25") %>%
  model(
    Mean = MEAN(tot_sales),
...
sales_fit %>%
  forecast(new_data(total_ts_sales %>% filter_index(.~"2015-04-25"), n = 28)) %>%
...
fc %>%
  filter(is_aggregated(item_id)) %>%
  autoplot(
    sales_aggr %>% filter_index("2015-04-15"~"2015-05")
...
lapply(a[5:9], mean)
#0.8698815 MASE for individual prediction -> not bad
#RMSSE  = 0.8698815 -> quite good!
...
```{r message=TRUE, warning=TRUE}
#ARIMA BOTTOM UP
fit_aggr_bu <- sales_aggr %>%
  filter_index(.~"2015-04-25") %>% 
  model(base = ARIMA(sales)) %>% reconcile(bu = bottom_up(base))


fc.fit_bu<- fit_aggr_bu %>% forecast(h = 28)
...
fit_aggr_etssss <- sales_aggr %>%
  filter_index(.~"2016-04-25") %>% 
  model(base = ETS(sales)) %>% reconcile(bu = bottom_up(base))


fc.fit_etssss <- fit_aggr_etssss %>% forecast(h = 28)

fc.fit_etssss %>%
  filter(is_aggregated(item_id)) %>%
  autoplot(
    sales_aggr %>% filter_index("2015-04-20"~"2015-05"))

fc.fit_etssss %>% filter(item_id == "FOODS_3_090") %>% 
  autoplot(
    sales_aggr %>% filter_index("2015-04-20"~"2015-05"))
...
fc.fit_etssss %>%
  ...
  accuracy(data = sales_aggr)
# A tibble: 4 x 12
#.model store_id     item_id      .type    ME  RMSE   MAE    MPE  MAPE  MASE RMSSE  ACF1
#<chr>  <chr*>       <chr*>       <chr> <dbl> <dbl> <dbl>  <dbl> <dbl> <dbl> <dbl> <dbl>
#  1 base   TX_2         <aggregated> Test   8.72  208.  167. -0.165  7.11 0.536 0.523 0.145
#2 base   <aggregated> <aggregated> Test   8.72  208.  167. -0.165  7.11 0.536 0.523 0.145
#3 td     TX_2         <aggregated> Test  -7.78  201.  163. -1.25   7.08 0.522 0.504 0.206
#4 td     <aggregated> <aggregated> Test  -7.78  201.  163. -1.25   7.08 0.522 0.504 0.206
```

```{r message=TRUE, warning=TRUE}
fit_aggr_tslm <- sales_aggr %>%
  filter_index(.~"2015-04-25") %>% 
  model(base = TSLM(sales ~ trend() + season())) %>% reconcile(bu = bottom_up(base))

fc.fit_tslm  <- fit_aggr_tslm %>% forecast(h = 28)
...
#.model store_id     item_id      .type    ME  RMSE   MAE    MPE  MAPE  MASE RMSSE  ACF1
#<chr>  <chr*>       <chr*>       <chr> <dbl> <dbl> <dbl>  <dbl> <dbl> <dbl> <dbl> <dbl>
#  1 base   TX_2         <aggregated> Test  -8.33  204.  165. -0.909  7.08 0.529 0.512 0.157
#2 base   <aggregated> <aggregated> Test  -8.33  204.  165. -0.909  7.08 0.529 0.512 0.157
#3 td     TX_2         <aggregated> Test  -8.33  204.  165. -0.909  7.08 0.529 0.512 0.157
#4 td     <aggregated> <aggregated> Test  -8.33  204.  165. -0.909  7.08 0.529 0.512 0.157
```

#MODEL FITTING

```{r message=TRUE, warning=TRUE}
#MODEL FITTING for item
