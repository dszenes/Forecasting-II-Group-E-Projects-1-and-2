...
```{r}
#Putting the different models together to compare their forecasts
all_models <- df %>%
          model(tslm = TSLM(tot_sales ~ trend() + season() + snap),
                arima1 = ARIMA(tot_sales ~ trend() + season() + snap),
                arima2 =ARIMA(tot_sales ~ snap))

all_models_fc <- all_models %>% forecast(new_data = future_souvenirs) 
all_models_fc %>% autoplot(total_ts_sales  %>% filter_index("2016"~.))
...
sales_aggr <- df2 %>%
  filter_index("2016-01-01"~"2016-04-25") %>% #remove the future cal date
...
#there is something that i dont understand, the code is wrong...
